...
Status: **current as of M0 and M1 complete; M2-M6 in progress, and the review-driven fixes R1-R5 and R8-R10 done** (2026-09-19). Milestones sequence the work described in
...
| M2 | T13 visual polish pass (Today, Month, Year, Day detail, widgets) | ⬜ after the foundation. A Compose performance pass already landed separately (see the CHANGELOG), so this task is about how it looks, not how it runs. Owner notes from device testing (2026-09-18/19): (1) the dark theme reads as plain black and white and is uninviting — bring the brand palette (teal / cream / amber) and colour into the surfaces. **Restated 2026-09-19, still the owner's biggest complaint after a device session: "the black and white view everywhere is draining my soul."** Catalogued as [FEATURES.md](FEATURES.md) W6, which lists where the colour should land (tinted containers for cards and tiles, a coloured Today hero, a consistent intercalary accent, more than one grey in the grids) and the two constraints: it must hold under dynamic colour as well as the brand fallback, and colour is never the only signal (Q4); (2) the month grid floats mid-screen with dead space above and below — anchor it and use the space; (3) the Month screen shows the month and year twice (app bar and grid heading) — keep one; (4) nothing signals that the app-bar title opens the Year view — add an affordance (chevron, chip or a dedicated action); (5) the Year view's mini-months are blank squares — consider day numbers or at least weekday structure, and check the top edge where a row is clipped under the app bar; (6) the Month widget's today highlight is a filled pill, not the app's ring; (7) ✅ done 2026-09-19, not waiting for this pass: a tall Month widget kept its grid in the top third with empty space below — the grid rows now carry a vertical weight so the grid scales to any height, each cell gained its Gregorian day number (the two weekday header rows were promising a second date the cells did not show), and the Gregorian span moved under the title. The Today widget looks right. Design reference for later: FEATURES C10, a Gregorian month grid with the IFC date inset (the reverse of our grid), as an optional view. Deliberately sequenced after events, converter and widgets The external review's UI notes are accepted into this task with verdicts in [reviews/2026-09-19-astra-analysis-response.md](reviews/2026-09-19-astra-analysis-response.md): secondary Gregorian text in cells, a selected-date summary under the grid, a dedicated Year Day / Leap Day treatment and accent, tighter Year tiles, large widgets that show more instead of whitespace, the "actual" vs "Gregorian" weekday label question. Acceptance test for the pass: unfamiliar users can answer "what Gregorian date is this?" and "when will this event happen?" |
...
| R6 | First screenshot baseline (the rest of M2 T10): tracked `roborazzi` output directory, preview-scanner captures for `:core:designsystem` and the main screens, goldens recorded in CI, `verifyRoborazziDebug` in the gate | 🟡 pipeline done, baseline not recorded: goldens live in a tracked `src/test/screenshots/` per module; `:core:designsystem` generates 56 captures from its previews with Roborazzi's preview scanner; `ci.yml` runs `verifyRoborazziDebug` only once a golden is tracked. **Owner step:** record and commit the first goldens ([screenshots.md](screenshots.md)) — sensibly after the M2 T13 design pass, or now for regression cover during it. Then add captures for the features and `:widget` |
