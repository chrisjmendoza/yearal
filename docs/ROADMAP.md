...
Status: **current as of M0 and M1 complete; M2-M6 in progress, M2 T13 (visual design pass) done, and the review-driven fixes R1-R5 and R7-R12 done** (2026-09-26). Milestones sequence the work described in
...
| M0 | T4 module stubs plus the dependency rule check | ✅ module stubs: `:app`, `:core:designsystem`, `:core:navigation`, `:core:scheduling`, `:feature:calendar`, `:feature:converter`, `:feature:events`, `:widget` exist; remaining modules are created by the task that first needs them. Dependency rule check: `DependencyRules.kt`'s `forbidProjectDependencies` (`ifc.android.library`, inherited by `ifc.android.feature`) fails any Android library module — including `:widget` and `:core:scheduling`, not only `:feature:*` — that depends on a `:feature:*` module or on `:core:data`; `restrictProjectDependenciesTo` (`ifc.jvm.library`) fails any pure-JVM module that depends on anything other than another pure-JVM module, and gives `:core:calendar` an empty allow-list (zero project dependencies, CLAUDE.md rule 1). Verified empirically for each branch: a forbidden dependency added to `:core:designsystem`, `:core:calendar`, `:core:holidays` and `:widget` in turn, each failing with the cited message, then reverted |
...
| M2 | T6 Today | ✅ done: hero date (IFC weekday above it since R12), both weekdays, day/week/quarter, year progress, countdown; the agenda and next-holiday cards shipped with M4 T7 and M6 |
...
| M2 | T12 privacy policy, in-app Privacy screen, Data safety form | 🟡 partial: the in-app Privacy screen is done (every claim sourced from [security-and-privacy.md](security-and-privacy.md) and the merged manifest); the hosted policy text ([privacy-policy.md](privacy-policy.md)) and the Data safety form answers ([play-data-safety.md](play-data-safety.md)) are drafted (2026-09-25). **Owner steps left:** publish the policy to GitHub Pages, put the URL in the Privacy screen's `privacy_policy_body` string and in Play Console, submit the Data safety form from the answers doc — with T11 |
...
| M5 | T4 widget config activity and per-widget state | ⬜ superseded, not planned: the design pass (M2 T13, Wave 3 I3) made both widgets follow the app's own appearance settings with a per-widget-type theme and opacity override in Settings, so there is no config activity and no per-instance Glance state (ARCHITECTURE §5). Revisit only if the 1.1 agenda widget needs per-instance options |
...
| M8 | T1 accessibility audit | 🟡 code-review pass done 2026-09-26: 29 findings (1 🔴, 16 🟠, 12 🟡) across every module, 28 fixed with tests in the same change — merged TalkBack descriptions on grid cells, the intercalary band and Year tiles (`mergeDescendants`, now pinned by zero-children tests); explicit 48dp touch targets on every bare `IconButton`/`TextButton` and dialog button (calendar, events, holidays, More); invalid editor dates announced through `SemanticsProperties.Error`; the converter result and editor banners as polite live regions; single-node year progress on Today; `heading()` on Settings group headings; intro pages reset scroll and focus; Sol block differs in shape in the intro illustration; per-cell descriptions on the Month widget, larger widget minimums and the low-opacity chip on the Month widget; `onSurfaceVariant` now contrast-tested on every container tier; 200% font-scale tests on Today, Settings and More. Deferred: #22 reduced motion (no plumbing exists; the pager animation is the only case) and #29 (widget-picker preview at 200%, device-only). **Owner step:** the TalkBack script, device-test-matrix.md T1–T16 |
| M8 | T5 migration test for schema v1 frozen | ✅ done: `SchemaFreezeMigrationTest` (`androidx.room3:room3-testing`'s `MigrationTestHelper`) validates the compiled `YearalDatabase` against the exported `1.json`; `LegacyDatabaseOpenTest` writes one row per table by raw SQL against the frozen v1 columns and reads it back through the real production builder; `SchemaExportGuardTest` guards the exported schema directory itself. Room's own `copyRoomSchemas` task overwrites the checked-in JSON to match a same-version entity edit before any test runs, so `LegacyDatabaseOpenTest` — not the freeze test alone — is what actually fails (Room's own "forgot to update the version number" error) when someone edits an entity without bumping the version; verified by temporarily adding, then separately renaming, a column |
| M8 | T6 "Send feedback" — bug reporting without a backend, an email intent with no server | ✅ A More-hub row (`feature/settings/more`, after Privacy) opens an `ACTION_SENDTO` `mailto:` chooser prefilled with a version-stamped subject and a body from the new pure `buildFeedbackBody` (device/app/settings diagnostics only, never event or holiday-pack content — CLAUDE.md rule 8); the address lives once, in the `feedback_email` string resource; no email app resolving shows the address as inline selectable text instead of crashing. `MoreViewModel` supplies the current `UserSettings` for the diagnostics; `:app` gained one new `versionCode` argument at the `MoreRoute` call site. Documented in [security-and-privacy.md](security-and-privacy.md) §6.3 and §7 |
...
| R7 | Make `robolectric.properties` (`sdk=36`) a convention-plugin default instead of a file every new Android module must remember to copy — verify empirically that the mechanism chosen really pins the SDK before deleting the eight hand-written copies | ✅ `configureIfcAndroid` generates `sdk=<catalog targetSdk>` into a directory it prepends onto every `Test` task's `classpath`. A `robolectric.<key>` system property does not work for `sdk` (only Robolectric's enum-valued configs read one) and neither does a `testImplementation` file dependency (AGP's variant model drops it before the resolved unit-test runtime classpath) — both confirmed empirically by deleting `:feature:calendar`'s copy first and watching its Compose tests fail until the classpath mutation actually worked; `:core:designsystem`'s `RobolectricSdkPinTest` now pins the guarantee permanently. All eight hand-written copies are deleted |
...
- **[C] (after B, all in `:feature:calendar`, so one agent or strictly separate files):**
  - T6: Today.
  - T7: Month pager.
  - T8: Day detail sheet (later folded into the Month day card, R12).
...
- **T12:** privacy policy on GitHub Pages, in-app Privacy screen, and the Data safety form. Play requires these before any track, including internal and closed testing. Drafted: [privacy-policy.md](privacy-policy.md) (end-user text) and [play-data-safety.md](play-data-safety.md) (form answers). Remaining: publish, wire the URL into the Privacy screen and Play Console, submit the form.
...
- **[B]:**
  ...
  - T4: ~~config activity plus per-widget state~~ — superseded by the design pass (M2 T13, Wave 3 I3): widgets follow the app's appearance with a per-widget-*type* override in Settings, and there is no config activity or per-instance state (ARCHITECTURE §5 "Widgets follow the app's appearance").
...
- **[A]:**
  - T1: 🟡 a11y audit (TalkBack script, font 200%, contrast) — the code-review pass and its fixes are done (see the ledger row); the on-device TalkBack pass ([device-test-matrix.md](device-test-matrix.md) "TalkBack") is the owner's step.
  ...
  - T5: ✅ migration test for schema v1 frozen; see the progress ledger row above.
...
- Add the Day / Month / Year view-mode switcher ([FEATURES.md](FEATURES.md) C13), so the three scales are one
  ...
  so wants an ADR. **Drafted 2026-09-25:** [adr/0006-calendar-view-switcher.md](adr/0006-calendar-view-switcher.md)
  (proposed; four decisions pre-filled for the owner — one `CalendarKey`, mode persisted in `UserSettings`,
  Month/Year only since the day popup is gone, a segmented control in the app bar).
- Add translations.
