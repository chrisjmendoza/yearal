...
| 🔴 | C5 | Tap a date → day detail (IFC date, Gregorian date, both weekdays, events, holidays) | The owner's core ask and a direct review request. Shown in the card below the month grid, not a popup (owner, 2026-09-25: the sheet only repeated the card). |
...
| 🟡 | C13 | **View-mode switcher: Day / Month / Year**, for drilling down to a date quickly | Owner-requested 2026-09-19. Makes the three scales one switchable view of a single selected date instead of three destinations reached different ways. Month is C1 and Year is C6 (both built) and Day reuses C5's day detail rather than becoming a second day UI, so the new build is the switching itself, not the views. **Selection is the invariant:** switching mode never changes the selected date, and picking a date in a coarser mode drills into it — that is the whole point of the feature. A Week mode was considered and left out: C9 stays ⚪ for the reason given there. Three things to settle before building: (1) whether mode becomes part of one `CalendarKey` or stays separate `:core:navigation` keys — it changes the tab's navigation shape, so it is ADR-worthy; (2) whether the mode persists in `SettingsRepository` like the other display settings or resets each launch; (3) ~~whether Day mode replaces the day-detail sheet~~ — moot since 2026-09-25: there is no sheet, the day detail is the Month screen's day card. This also subsumes the M2 T13 note that nothing signals the app-bar title opens the Year view — a switcher is that affordance. **ADR drafted:** [adr/0006-calendar-view-switcher.md](adr/0006-calendar-view-switcher.md) (2026-09-25, proposed, awaiting the owner's answers). Overlaps C7 (jump to date) and the `IfcDatePicker`: those stay the *precise* path to a date, this is the *browsing* path. |
| 🟡 | C10 | Gregorian month grid with IFC dates overlaid (the inverse view) | For planning around Gregorian-world deadlines. Owner-requested as an optional view (2026-09-19): the mirror of C1 — Gregorian day large, IFC date small. **Unchanged by the M2 T13 design pass** — the pass restyled the existing screens on the new tokens but did not build this view; still unscheduled. |
...
| 🟠 | L3 | Contextual explainers (info icon on the nominal-weekday header and on floating-day rows) | **Done.** `ExplainerInfoButton` (`:core:designsystem`, package `explainer`): a 48dp info button opening a short `AlertDialog`; title/explanation are caller-supplied strings, so it carries no calendar copy of its own. `MonthGrid` calls it twice — beside the month heading, explaining the two kinds of weekday, and beside the intercalary band, explaining why one row spans all seven columns. Neither sits inside the row it explains, which would break the grid's column alignment; see `docs/ARCHITECTURE.md` §4 "Screens and navigation" → "Contextual explainers". |
...
| 🟠 | W2 | Theme: system / light / dark; colour source (Yearal palette / Material You); six curated palettes (Teal, Sol, Night, Moss, Rose, Ink); pure black for dark mode | **Delivered by ROADMAP M2 T13.** The incumbent has a single red theme; Material You is opt-in (below API 31 it is shown disabled), the Yearal palette is the default. |
| 🟠 | W6 | **Colour through the UI, not just in the accents** | **Delivered by ROADMAP M2 T13** ([design-plan.md](design-plan.md)). Owner, 2026-09-19, on a device: "the black and white view everywhere is draining my soul." The palette existed (`IfcTheme`, teal / cream / amber) but the screens spent it almost nowhere — in dark mode nearly every surface landed on near-black with white text, so the app read as monochrome even though the theme was not. Now: tinted containers for cards and tiles, a coloured Today hero, the intercalary days sharing one consistent accent across Month, Year, Day detail and both widgets, and the Year and Month grids using more than one grey. Holds under both the Yearal palette and Material You, and colour is never the only signal (Q4) — gated by `ColorSchemeContrastTest`. |
| 🟠 | W6 | "Delete all data" | |
...
| 🟠 | W9 | Per-event colour and category | **Delivered by ROADMAP M2 T13.** Same feature as E8 below — the editor's colour-swatch row (seven fixed hues plus "Calendar colour") and the `EventCategory` segmented control, both already stored per `docs/contracts/Events.md`. Catalogued here too since it shipped as part of the Appearance/colour work, not a separate events task. |
| 🟡 | W10 | Per-widget theme (follow app / light / dark) and background-opacity setting | **Delivered by ROADMAP M2 T13.** Global per widget type, not per placed instance (a configure activity and per-`GlanceId` state can come later). Known gap: a widget's own theme override has no effect while the colour source is Material You, since Glance's dynamic colour set follows the system UI mode. |
| 🟡 | W11 | Live palette preview strip in Settings | **Delivered by ROADMAP M2 T13.** A miniature 7-day row (today ring, one holiday diamond) drawn with the design system's own components, next to the theme and palette controls, so a choice is visible before committing. |
...
| 🟠 | P11 | "Send feedback" — bug reporting without a backend, an `ACTION_SENDTO` email intent with device/app/settings diagnostics, never event or holiday-pack content | **Delivered M8 T6.** More hub, `:feature:settings`. |
